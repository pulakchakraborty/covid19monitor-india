...
const InfectionsChart = lazy(() => import('./InfectionsChart'));
            ...
            setIndiaHistorical(
              responseHistory.data.slice(1).map((data, index) => {
                return {
                  ...
                  newCases:
                    data.summary.total -
                    responseHistory.data[index].summary.total,
                  newRecoveries:
                    data.summary.discharged -
                    responseHistory.data[index].summary.discharged,
                  newDeaths:
                    data.summary.deaths -
                    responseHistory.data[index].summary.deaths,
                };
              })
            );
          }
  ...
  return (
    <Styles>
      ...
      <Suspense fallback={renderLoader()}>
        {!mapIsIndia && chartName === 'Daily Cases' && (
          <DailyChart chartData={allHistorical} />
        )}
        {mapIsIndia && chartName === 'Daily Cases' && (
          <DailyChart chartData={indiaHistorical} />
        )}
        {!mapIsIndia && chartName === 'Daily Deaths' && (
          <DailyDeathsChart chartData={allHistorical} />
        )}
        {mapIsIndia && chartName === 'Daily Deaths' && (
          <DailyDeathsChart chartData={indiaHistorical} />
        )}
        {!mapIsIndia && chartName === 'Total Cases' && (
          <CasesChart chartData={allHistorical} />
        )}
        {mapIsIndia && chartName === 'Total Cases' && (
          <CasesChart chartData={indiaHistorical} />
        )}
        {!mapIsIndia && chartName === 'Active Cases' && (
          <ActiveCasesChart chartData={allHistorical} />
        )}
        {mapIsIndia && chartName === 'Active Cases' && (
          <ActiveCasesChart chartData={indiaHistorical} />
        )}
      </Suspense>
      <Suspense fallback={renderLoader()}>
        {mapIsIndia ? (
